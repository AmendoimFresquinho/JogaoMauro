    ...
    int xReiniciar = 50;
     ...
     while (!WindowShouldClose()) { 
       BeginDrawing();
        ...
        DesenharMapa(&mapa);  
        DrawRectangle(mario.corpoFisico.posX, mario.corpoFisico.posY, mario.tamanho/2, mario.tamanho/2, BLUE);
...
void mover(Jogador *p, Mapa* mapa){
    testarColisaoY(&(p->corpoFisico), &mapa->grid[0][0], TAMANHO_BLOCOS, COLUNAS, LINHAS);
...
Rectangle player = {j->corpoFisico.posX, j->corpoFisico.posY, TAMANHO_JOGADOR, TAMANHO_JOGADOR}; // Retangulos invisiveis que servem para checar a colisao
